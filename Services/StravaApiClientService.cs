...
namespace StravaDiscordBot.Services
{
    ...
    public class StravaApiClientService : IStravaApiClientService
    {
        ...
        public async Task<List<DetailedActivity>> FetchActivities(string accessToken, DateTime after,
            int itemsPerPage = 100)
        {
            return await GetAsync<List<DetailedActivity>>(
                $"/api/v3/athlete/activities?after={(int) (after - new DateTime(1970, 1, 1)).TotalSeconds}&per_page={itemsPerPage}",
                accessToken);
        }
        ...
        {
            using (var http = new HttpClient {BaseAddress = new Uri(StravaBaseUrl)})
            {
                ...
                _logger.LogInformation(
                    $"Call to {GetUrlSuffixWithoutQuery(urlSuffix)} - {response.StatusCode} Status code");
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    throw new StravaException(StravaException.StravaErrorType.Unknown,
                        $"Failed to get from strava, status code: {response.StatusCode}");
                }
        ...
        public async Task<T> PostAsync<T>(string urlSuffix, string accessToken = null)
        {
            ...
            if (response.IsSuccessStatusCode) return JsonConvert.DeserializeObject<T>(responseContent);
            if (response.StatusCode == HttpStatusCode.Unauthorized)
                throw new StravaException(StravaException.StravaErrorType.Unauthorized, "Access token expired");
            ...
            _logger.LogError(responseContent);
            throw new StravaException(StravaException.StravaErrorType.Unknown,
                $"Failed to post to strava, status code: {response.StatusCode}");
        }
